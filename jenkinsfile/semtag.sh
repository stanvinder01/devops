#!/usr/bin/bash
trap 'exit' ERR
# Source of file: https://github.com/pnikosis/semtag Version: v0.1.0
...
function increase_version {
  ...
  if [ "$displayonly" == "true" ]; then
    ...
    if [ "$finalversion" == "$FIRST_VERSION" ] || [ "$hasversiontag" != "true" ]; then
      __commitlist="$(git log --pretty=oneline --abbrev-commit | cat)"
    else
      __commitlist="$(git log --pretty=oneline --abbrev-commit $finalversion... | cat)"
    fi
